...
\usepackage[american]{babel}          % American English hyphenation/spelling
...
% --- single spacing is the default; do not change line spacing ---

% --- captions: label bold, placed below floats (per house style) ---
\captionsetup{labelfont=bf,font=small,labelsep=period}

% --- headings: left-aligned, bold, 16 / 14 / 12 pt ---------------------------
\titleformat{\section}
  {\normalfont\fontsize{16}{19}\bfseries\raggedright}{\thesection}{0.6em}{}
\titleformat{\subsection}
  {\normalfont\fontsize{14}{17}\bfseries\raggedright}{\thesubsection}{0.6em}{}
\titleformat{\subsubsection}
  {\normalfont\fontsize{12}{14}\bfseries\raggedright}{\thesubsubsection}{0.6em}{}
\titlespacing*{\subsubsection}{0pt}{1.5ex}{0pt} % no extra space before body

% --- running head (first author) + right-aligned page numbers ----------------
\setlength{\headheight}{14.5pt}
\pagestyle{fancy}
\fancyhf{}
\fancyhead[R]{Kuar \& Williams}
\fancyfoot[R]{\thepage}
\renewcommand{\headrulewidth}{0pt}
\fancypagestyle{plain}{%        first page: no running head, page number still right
  \fancyhf{}\fancyfoot[R]{\thepage}\renewcommand{\headrulewidth}{0pt}}

% --- hanging-indent reference list -------------------------------------------
\newenvironment{references}
  {\section*{References}\small\setlength{\parindent}{0pt}%
   \setlength{\parskip}{0.6em}\everypar{\hangindent=0.5in \hangafter=1}}
  {\par}

\begin{document}

%==============================================================================
%  TITLE BLOCK  (course paper --- author info kept, not anonymized)
%==============================================================================
\begin{center}
  {\fontsize{18}{22}\bfseries Are NBA Draft Picks Worth the Price?\\[2pt]
   Surplus Value and the Analytics Era\par}
  \vspace{1.2em}
  {\normalsize Sharodeep Kuar \quad Caleb Williams\par}
  \vspace{0.4em}
  {\small Sports Finance: an Introduction, Waseda Business School\\
...
\subsection{Surplus Value in Sports Labor Markets}
...
\subsection{The NBA Rookie Scale and the 1995 CBA}
...
\subsection{The Loser's Curse and Overvaluing Draft Picks}
...
\subsection{Behavioral Economics and Draft Order Bias}
...
\subsection{The Analytics Revolution and Market Efficiency}

The development of advanced basketball statistics began in the late 1990s and early 2000s, though largely outside team front offices. Berri (1999) was one of the first to develop a formal model that converted player box score data directly into wins called Wins Produced. Berri argued that traditional basketball evaluation over-rewarded high scorers and ignored other contributions, meaning many players were being mispriced in the labor market. Dean Oliver's \emph{Basketball on Paper} (2004) introduced a parallel framework of efficiency-based ratings still in use today. These tools, however, were built by analysts and academics; league-wide adoption by teams came considerably later, roughly post-2010, which motivates our structural break at that point.
...
\toprule
\textbf{Model} & \textbf{Specification} & \textbf{Purpose} \\
\midrule
P1 & VORP/1M $\sim$ Pick & Baseline: does pick number predict cumulative value? \\
P2 & VORP/1M $\sim$ Pick + Post & Add era dummy, parallel slopes \\
P3 & VORP/1M $\sim$ Pick $\times$ Post & Main model: does the slope change by era? \\
P4 & VORP/1M $\sim$ log(Pick) $\times$ Post & Log-linear: diminishing returns to pick position \\
P5 & WS/1M $\sim$ Pick $\times$ Post & Win Shares robustness check \\
BD & VORP/1M $\sim$ Bin $\times$ Post & Bin-level convergence: did each bin's gap vs.\ Top 5 shrink? \\
\bottomrule
\end{tabular}
\end{center}

The interaction term in P3 (Pick $\times$ Post) is the central test of our hypothesis. A positive and significant interaction coefficient indicates that the negative relationship between pick number and cumulative value per dollar weakened in the post-analytics era. The bin dummy model (BD) complements P3 by testing directly whether each bin's gap relative to Top 5 shrank post-2010, without imposing a linear relationship across all pick positions.
...
\toprule
\textbf{Pick Bin} & \textbf{Era} & \textbf{N} & \textbf{VORP/1M} & \textbf{Std} & \textbf{Bust Rate} & \textbf{Sharpe Analog} \\
\midrule
...
\bottomrule
\end{tabular}
\caption{Summary statistics by draft position bin and era.}
\end{table}

\begin{figure}[ht]
\centering
\includegraphics[width=0.85\textwidth]{fig1_performance_per_dollar.png}
\caption{VORP per \$1M by draft bin and era.}
\end{figure}
...
Third, the top-five picks dominate all other bins on risk-adjusted return. The Sharpe analog for top-five picks was 0.69 pre-analytics and 0.50 post-analytics while for the second-round picks it is 0.27. Top picks also carry the lowest bust rate. 12.4 percent of the top-five player-seasons produced negative VORP pre-analytics. This figure rose to 19.0 percent post-analytics but is still well below the second-round bust rate of 33.3 percent. In mean-variance terms the top picks offer the highest return and the lowest volatility. This makes them the dominant asset class in every era under VORP.
...
\subsection{Era Convergence}

\begin{figure}[ht]
\centering
\includegraphics[width=0.85\textwidth]{fig2_pick_vs_vorp_scatter.png}
\caption{Pick number vs.\ VORP per \$1M with LOWESS trend.}
\end{figure}
...
\begin{figure}[ht]
\centering
\includegraphics[width=0.85\textwidth]{fig3_era_comparison_line.png}
\caption{Era comparison -- VORP per \$1M by draft bin.}
\end{figure}
...
\begin{figure}[ht]
\centering
\includegraphics[width=0.85\textwidth]{fig4_risk_return.png}
\caption{Risk--return scatter by draft bin and era.}
\end{figure}
...
\toprule
\textbf{Model} & \textbf{N} & \textbf{$R^2$} & \textbf{Pick coef.} & \textbf{p} & \textbf{Post coef.} & \textbf{p} & \textbf{Interaction} & \textbf{p} \\
\midrule
...
\bottomrule
\end{tabular}
\caption{OLS regression results, player-level rookie window sample ($N = 839$).}
\end{table}
...
The main model (P3) introduces the interaction between pick number and the post-analytics dummy. The interaction coefficient is 0.00462 (p = 0.031), indicating that the negative relationship between pick number and cumulative value per dollar weakened significantly in the post-analytics era. The value curve flattened after 2010. The log-linear specification (P4) confirms this: the interaction is 0.0749 (p = 0.020), indicating the same convergence under diminishing returns to pick position. The Win Shares robustness check (P5) does not find a significant interaction (p = 0.398), a divergence we address in Section 5.4.
...
\begin{tabular}{@{}lrrrr@{}}
\toprule
\textbf{Bin} & \textbf{Pre gap} & \textbf{p} & \textbf{Post change} & \textbf{p} \\
\midrule
...
\bottomrule
\end{tabular}
\caption{Bin dummy model (BD): gap vs.\ Top 5 pre-analytics and post-analytics change.}
\end{table}
...
\toprule
\textbf{Quantile} & \textbf{N} & \textbf{Pick coef.} & \textbf{p} & \textbf{Interaction} & \textbf{p} \\
\midrule
...
\bottomrule
\end{tabular}
\caption{Quantile regression results at 25th, 50th, and 75th percentiles.}
\end{table}
...
\bottomrule
\end{tabular}
\caption{Bin dummy model on full career data (robustness check).}
\end{table}
...
Third, we test whether the P3 interaction is robust to the differential rookie-window completeness discussed in Section 5.4. Adding the number of rookie-window seasons observed per player (\texttt{n\_seasons}, range 1--4) as a control to P3 yields:
...
\toprule
\textbf{Spec} & \textbf{N} & \textbf{$R^2$} & \textbf{Interaction} & \textbf{p} & \textbf{\texttt{n\_seasons}} & \textbf{p} \\
\midrule
P3 + n\_seasons & 839 & 0.140 & $+0.0056$ & 0.007 & $+0.127$ & $<0.001$ \\
\bottomrule
\end{tabular}
\caption{P3 with rookie-window completeness control.}
\end{table}

\texttt{n\_seasons} is itself a strong positive predictor of cumulative VORP/1M (more seasons captured means more value accumulated), and controlling for it \emph{strengthens} the era interaction (from p = 0.031 to p = 0.007) rather than weakening it. This suggests the P3 finding is not an artifact of the asymmetric data coverage across eras described in Section 5.4.

%==============================================================================
\section{Discussion}

\subsection{Draft Picks as Financial Assets}
...
\subsection{The Analytics Era and Market Convergence}

When measured over the rookie contract window at the player level, the surplus value curve flattened significantly after 2010. The interaction term in P3 is significant at p = 0.031, and the bin dummy model shows all three bins below Top 5 saw their pre-analytics gap narrow significantly post-2010: Picks 6--14 (p = 0.043), Picks 15--30 (p = 0.031), and the Second Round (p = 0.003). The convergence holds under the log-linear specification as well (P4, p = 0.020), and strengthens further once the rookie-window completeness control is added (p = 0.007; Section 4.4). This is consistent with the hypothesis that analytics-era teams became better at identifying value throughout the draft, reducing the information gap between top picks and the rest of the first round during the period when the CBA price is fixed.
...
%==============================================================================
\begin{references}
Berri, D. J. (1999). Who is ``most valuable''? Measuring the player's production of wins in the National Basketball Association. \emph{Managerial and Decision Economics, 20}(8), 411--427. \url{https://doi.org/10.1002/(SICI)1099-1468(199912)20:8<411::AID-MDE950>3.0.CO;2-G}

Berri, D. J., Schmidt, M. B., \& Brook, S. L. (2006). \emph{The wages of wins: Taking measure of the many myths in modern sport.} Stanford University Press.

Camerer, C. F., \& Weber, R. A. (1999). The econometrics and behavioral economics of escalation of commitment: A re-examination of Staw and Hoang's NBA data. \emph{Journal of Economic Behavior \& Organization, 39}(1), 59--82. \url{https://doi.org/10.1016/S0167-2681(99)00026-8}

Groothuis, P. A., Hill, J. R., \& Perri, T. J. (2007). Early entry in the NBA draft: The influence of unraveling, human capital, and option value. \emph{Journal of Sports Economics, 8}(3), 223--243. \url{https://doi.org/10.1177/1527002505281228}

Hakes, J. K., \& Sauer, R. D. (2006). An economic evaluation of the Moneyball hypothesis. \emph{Journal of Economic Perspectives, 20}(3), 173--186. \url{https://doi.org/10.1257/jep.20.3.173}

Massey, C., \& Thaler, R. H. (2013). The loser's curse: Decision making and market efficiency in the National Football League draft. \emph{Management Science, 59}(7), 1479--1495. \url{https://doi.org/10.1287/mnsc.1120.1657}

National Basketball Association \& National Basketball Players Association. (1995). \emph{Collective bargaining agreement.}

Oliver, D. (2004). \emph{Basketball on paper: Rules and tools for performance analysis.} Potomac Books.

Rottenberg, S. (1956). The baseball players' labor market. \emph{Journal of Political Economy, 64}(3), 242--258. \url{https://doi.org/10.1086/257790}

Scully, G. W. (1974). Pay and performance in Major League Baseball. \emph{The American Economic Review, 64}(6), 915--930.

Staw, B. M., \& Hoang, H. (1995). Sunk costs in the NBA: Why draft order affects playing time and survival in professional basketball. \emph{Administrative Science Quarterly, 40}(3), 474--494. \url{https://doi.org/10.2307/2393794}
\end{references}

%==============================================================================
\section*{Appendix}

\subsection*{A. Sample Data}
...
\toprule
\textbf{Player} & \textbf{Draft Yr} & \textbf{Pick} & \textbf{Bin} & \textbf{Era} & \textbf{Season} & \textbf{VORP} & \textbf{Salary (\$M)} & \textbf{VORP/1M} \\
\midrule
...
\subsection*{B. Data Coverage by Draft Cohort}
...
\begin{tabular}{@{}lrrr@{}}
\toprule
\textbf{Draft Year} & \textbf{Player-Seasons} & \textbf{Unique Players} & \textbf{Salary Coverage} \\
\midrule
1995 & 156 & 25 & 100\% \\
1996 & 214 & 28 & 100\% \\
1997 & 191 & 29 & 100\% \\
...
2018 & 175 & 41 & 100\% \\
\midrule
\textbf{Total} & \textbf{6,095} & \textbf{927} & \textbf{100\%} \\
\bottomrule
\end{tabular}
\end{center}
